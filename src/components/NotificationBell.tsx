...
function NotifRow({ item, onSnooze, onClose }: {
  ...
  // Severity sebagai border kiri — kentara, tetap ringan
...
export function NotificationBell() {
  ...
  const computePosition = useCallback(() => {
    ...
    // Cari ancestor dengan stacking context tinggi (sidebar pakai z-modal=300).
    // Anchor dropdown ke right edge container itu — supaya tidak overlap sidebar.
    let anchorRight = rect.right;
    let cur: HTMLElement | null = btnRef.current.parentElement;
    while (cur && cur !== document.body) {
      const cs = window.getComputedStyle(cur);
      const zNum = parseInt(cs.zIndex, 10);
      if (!isNaN(zNum) && zNum >= 100) {
        anchorRight = Math.max(anchorRight, cur.getBoundingClientRect().right);
        break;
      }
      cur = cur.parentElement;
    }

    // Desktop: anchor di samping sidebar. Dropdown grows upward dari bell.bottom.
      ...
      {open && dropPos && createPortal(
        <div
          ...
          style={{
            ...
            zIndex: 'var(--z-popover)', // di atas sidebar (z-modal=300) supaya tidak tertutup
          }}
